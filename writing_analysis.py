...
def operator_releases_summary_stats(operator):
    ...
    pass_cm_qc = pass_su_qc.loc[pass_su_qc.operator_kept == True]
    print(f'{len(pass_cm_qc)} overpasses quantified by {operator}')
    fail_cm_qc = pass_su_qc.loc[pass_su_qc.operator_kept == False]
    print(f'{len(fail_cm_qc)} overpasses removed by {operator}\n')
    ...
    # Find largest release given by Stanford that passes SU QC
    max_info = pass_su_qc.loc[pass_su_qc.release_rate_kgh.idxmax()][relevant_cols]
    print(f'Largest volume overpass for {operator}:')
    print_overpass_info(max_info)


    # Find smallest non-zero release to pass Stanford QC
    non_zero_overpasses = pass_su_qc.loc[pass_su_qc.non_zero_release == True]
    min_info = non_zero_overpasses.loc[non_zero_overpasses.release_rate_kgh.idxmin()][relevant_cols]
    print(f'Smallest non-zero volume overpass for {operator}:')
    print_overpass_info(min_info)

    # Zero releases
    zero_releases = pass_su_qc.loc[pass_su_qc.zero_release == True]
    ...
    if false_negatives.empty:
        ...
        print(f'False positives detected: {len(false_negatives)}\n')

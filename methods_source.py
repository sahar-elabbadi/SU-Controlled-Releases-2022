...
def generate_overpass_summary(operator, stage, timekeeper='flightradar', strict_discard=False,
    ...
    if op_ab == 'sciav':
        overpass_summary['overpass_datetime_end'] = combined_df.datetime_end_utc
...
def load_overpass_summary(operator, stage, strict_discard=False, time_ave=60, gas_comp_source='ms'):
    ...
    if op_ab == 'sciav':
        # Import overpass_datetime_end as date
        overpass_summary = pd.read_csv(save_path, index_col=0, parse_dates=['overpass_datetime', 'overpass_datetime_end'])
...
def summarize_wind_conditions(start_t, stop_t):
    ...
    if start_t.date() != stop_t.date():
        ...
        if not file_path.is_file():
            ...
            average_winddirection = circmean(
                pd.to_numeric(average_period['winddirection'], errors='coerce') / radians_to_degrees,
                nan_policy='omit') * radians_to_degrees  # .mean()
            stdev_windspeed = average_period['windspeed'].std()
            stdev_winddirection = circstd(
                pd.to_numeric(average_period['winddirection'], errors='coerce') / radians_to_degrees,
                nan_policy='omit') * radians_to_degrees  # .std()
